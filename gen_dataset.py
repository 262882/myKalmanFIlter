...
def gen_dataset(gt_signal:torch.Tensor, out_name:str, num_train:int = 1000, num_cv:int = 100, num_test:int = 200, Q_mag=0.5):
    ...
    for cnt, seq in enumerate(train_gt):
        ...
        train_gt[cnt][0] = train_gt[cnt][0] #+ 5*2*(random.random()-0.5)
    ...
    for cnt, seq in enumerate(cv_gt):
        ...
        cv_gt[cnt][0] = cv_gt[cnt][0] #+ 5*2*(random.random()-0.5)
    ...
    for cnt, seq in enumerate(test_gt):
        ...
        test_gt[cnt][0] = test_gt[cnt][0] #+ 5*2*(random.random()-0.5)
...
if __name__ == "__main__":
    ...
    gen_dataset(para, './datasets/para.pt', Q_mag=1000)
